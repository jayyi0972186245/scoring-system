...
> ⚠️ 注意：`interviewQueue` 節點必須包含在規則內，否則面試掃碼頁面無法推送考生給面試老師。
...
工作人員帳號需在管理後台手動新增，**不可**使用老師密碼登入工作人員頁面。
...
> ⚠️ 面試工作人員必須選 CM316，否則無法登入面試掃碼頁面。

---

## 第八步：測試系統

1. 打開你的網址，應看到管理後台登入頁面
2. 輸入管理員密碼登入
3. 匯入或新增一位測試考生
4. 點擊「QR」按鈕，查看考生 QR 碼
5. 用手機開啟 `你的網址/interview-scan.html`，以面試工作人員密碼登入，測試掃碼與推送
6. 用另一台裝置開啟 `你的網址/interview.html`，確認考生出現在左側面板
...
| 人員 | 網址 | 密碼 |
|------|------|------|
| 管理員 | `你的網址/index.html` | 請洽管理員 |
| 考生報到 | `你的網址/checkin.html` | 不需密碼（輸入身分證末四碼） |
| 面試工作人員（CM316） | `你的網址/interview-scan.html` | 請洽管理員 |
| 術科工作人員 | `你的網址/scan.html` | 請洽管理員 |
| 面試老師（CM316） | `你的網址/interview.html` | 請洽管理員 |
| 術科老師 | `你的網址/practical.html` | 請洽管理員 |
| 成績總表 | `你的網址/results.html` | 請洽管理員 |
...
1. **面試工作人員**在 CM316 門口，用 `interview-scan.html` 掃描考生 QR 碼（或輸入序號末三碼）
2. 每次可加入 1–3 位考生至「本組暫存區」
3. 確認人員後按「**推送這組考生至面試評分**」
4. **面試老師**在 `interview.html` 會看到左側出現新的一組，點選後右側同時顯示所有考生的評分欄
5. 老師逐一輸入每位考生的分數並送出，完成後該組移至「已完成」區
...
**Q：面試掃碼推送後面試老師沒看到考生？**
A：最常見原因是 Firebase 規則缺少 `interviewQueue` 節點。請確認規則已包含第二步中的完整規則並發布。

**Q：工作人員無法登入面試掃碼頁面？**
A：確認管理後台「工作人員管理」中，該工作人員的負責教室設定為 **CM316**，只有 CM316 的工作人員才能登入 `interview-scan.html`。
...
**Q：考試結束後要匯出資料嗎？**
A：進管理後台 → 「🛠️ 工具」分頁 → 點擊「**匯出成績 CSV**」，可以直接在 Excel 開啟。CSV 欄位包含面試委員1、面試委員2、評分、術科評分委員1、術科評分委員2、成績及平均分數。
